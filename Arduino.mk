...
#
# Copyright Rob O'Dwyer <odwyerrob@gmail.com>, based on work that is
...
#
#         0.5  2.vi.2011   Rob O'Dwyer
#                          - improved support for compiling libraries with source files
#                          - fixed library path (hardware/libraries -> libraries)
#                          - added support for setting ARD_PORT directly 
#                          		(e.g. for usb programmers that don't correspond to a file)
#
#
########################################################################
#
...
# Expand and pick the first port unless specified manually
ifndef ARD_PORT
...
define run-monitor
	...
	@read -p "Press <Ctrl-a><Ctrl-\> to exit "
	screen $(ARDUINO_PORT) $(MON_SPEED)
endef
define kill-monitor
	@- pkill -f 'screen.*$(ARDUINO_PORT)'
endef
...
$(OBJDIR)/%.o: $(ARDUINO_CORE_PATH)/%.c
	@$(ECHO) "Building core file $(notdir $<)"
	@$(CC) -c $(CPPFLAGS) $(CFLAGS) $< -o $@
...
AVRDUDE_ISP_OPTS = -P $(ISP_PORT) -c $(ISP_PROG)
